defmodule Kaguya.Module.Core do
  ...
  defh changeUserNick(%{trailing: new_nick, user: %{nick: old_nick}}) do
    for member <- :pg.get_members(:default, :channels), do: GenServer.call(member, {:rename_user, {old_nick, new_nick}})
  end
  ...
  defh removeNickFromAllChans(%{user: %{nick: nick}}) do
    for member <- :pg.get_members(:default, :channels), do: GenServer.call(member, {:del_user, nick})
  end
